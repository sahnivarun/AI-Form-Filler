...
class CustomGenAIModel(LLM):
    ...
    def _call(self, prompt: str, stop=None) -> str:
        try:
            # Use the actual method for the GenerativeModel
            response = self._model.generate_content(prompt)
            return response.text
            #return response.candidates[0]['output']  # Update key based on library response
        except AttributeError:
            raise ValueError("The GenerativeModel object does not support the 'predict' method. Check the library's documentation.")
...
def get_llm():
    # Set your API key
    os.environ["API_KEY"] = "AIzaSyAHhH7Rh3_qZ0d23fZu1_Qv_OunjC8bjoU"  # Replace with your actual API key here
...
def process_data():
    ...
    # Initialize HuggingFaceEmbeddings with model identifier string
...
# API endpoint for Chrome Extension to retrieve form data
